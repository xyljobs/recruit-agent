...
**人才决策Agent人才智能匹配系统** - 一款可私有部署的 AI 驱动 HR 招聘决策副驾驶，帮助 HR 高效完成 JD 解析、候选人匹配、短名单决策、沟通话术生成、结果复盘和外部渠道（Boss直聘）候选人搜索。默认以纯规则模式（`AI_EXECUTION_MODE=rules_only`）运行，AI 能力需经部署级 + 租户级双重审批后方可启用。
